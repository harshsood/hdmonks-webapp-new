...
class EmailService:
    ...
    def send_contact_inquiry_notification(self, inquiry: Dict[str, Any]) -> bool:
        ...
        try:
            customer_name = inquiry.get('full_name') or inquiry.get('name', 'Customer')
            subject = f"New Contact Inquiry from {customer_name}"
            
            body = f"""
            <h2>New Contact Inquiry</h2>
            <p><strong>Name:</strong> {customer_name}</p>
            <p><strong>Email:</strong> {inquiry.get('email', 'Not provided')}</p>
            ...
            <p>{inquiry.get('message', 'No message provided')}</p>
            <p><strong>Submitted:</strong> {inquiry.get('created_at', 'Unknown')}</p>
            """
            ...
            customer_sent = self.send_email(inquiry.get('email', ''), customer_subject, customer_body, is_html=True)
            
            return admin_sent or customer_sent
        except Exception as e:
            logger.error(f"Error sending contact inquiry notification: {str(e)}")
            return False
    ...
    def send_booking_confirmation(self, booking: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            subject = f"Consultation Booking Confirmed - {booking.get('date', 'TBD')} at {booking.get('time', 'TBD')}"
            
            # Send to admin
            admin_body = f"""
            <h2>New Consultation Booking</h2>
            <p><strong>Name:</strong> {customer_name}</p>
            <p><strong>Email:</strong> {booking.get('email', 'Not provided')}</p>
            <p><strong>Phone:</strong> {booking.get('phone', 'Not provided')}</p>
            <p><strong>Company:</strong> {booking.get('company', 'Not provided')}</p>
            <p><strong>Service Interest:</strong> {booking.get('service_interest', 'Not specified')}</p>
            <p><strong>Date & Time:</strong> {booking.get('date', 'TBD')} at {booking.get('time', 'TBD')}</p>
            ...
            <p><strong>Date:</strong> {booking.get('date', 'TBD')}</p>
            <p><strong>Time:</strong> {booking.get('time', 'TBD')}</p>
            <p><strong>Service:</strong> {booking.get('service_interest', 'Not specified')}</p>
            <p><strong>Booking ID:</strong> {booking.get('id', 'N/A')}</p>
            ...
            customer_sent = self.send_email(booking.get('email', ''), subject, customer_body, is_html=True)
    ...
    def send_booking_reminder(self, booking: Dict[str, Any]) -> bool:
        ...
        try:
            customer_name = booking.get("full_name") or booking.get("name", "Customer")
            subject = f"Reminder: Your consultation tomorrow at {booking.get('time', 'TBD')}"
            ...
            return self.send_email(booking.get('email', ''), subject, body, is_html=True)
        except Exception as e:
