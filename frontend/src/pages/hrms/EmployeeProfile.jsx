...
const EmployeeProfile = () => {
  ...
  const createAccount = async (event) => {
    ...
    if (username.length < 3 || password.length < 8) {
      setError('Use a username with at least 3 characters and a password with at least 8 characters.');
      return;
    }
    setAccountSaving(true);
    try {
      await axios.post(`${API}/${id}/account`, { username, password }, { headers });
      setAccountForm({ username: '', password: '' });
      setMessage('Login access created. The employee can now sign in at /hrms/login.');
      await loadProfile();
    } catch (accountError) {
      setError(accountError.response?.data?.detail || (accountError.request ? 'The server could not be reached. Please refresh and try again.' : 'Unable to create login access.'));
    } finally {
