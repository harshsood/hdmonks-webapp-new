...
const LeaveManagement = () => {
  ...
  return (
    <section className="space-y-6">
      ...
      {isApply && <form onSubmit={apply} className="max-w-3xl rounded-xl border border-gray-200 bg-white p-6 shadow-sm"><h2 className="text-xl font-semibold text-gray-900">Apply for leave</h2>{!leaveTypes.length && <p className="mt-4 rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">Leave policies are not configured yet. Please contact HR.</p>}<div className="mt-5 grid gap-4 md:grid-cols-2"><label className="block text-sm"><span className="mb-1 block font-medium">Leave type</span><select disabled={!leaveTypes.length} value={form.leave_type} onChange={(event) => setForm({ ...form, leave_type: event.target.value })} className="w-full rounded-lg border px-3 py-2.5">{leaveTypes.map((item) => <option key={item.code}>{item.code}</option>)}</select></label><label className="block text-sm"><span className="mb-1 block font-medium">Duration</span><select disabled={!leaveTypes.length} value={form.duration} onChange={(event) => setForm({ ...form, duration: event.target.value })} className="w-full rounded-lg border px-3 py-2.5"><option value="full_day">Full day</option><option value="half_day">Half day</option><option value="hours">Hours</option></select></label><label className="block text-sm"><span className="mb-1 block font-medium">Start date</span><input disabled={!leaveTypes.length} required type="date" value={form.start_date} onChange={(event) => setForm({ ...form, start_date: event.target.value })} className="w-full rounded-lg border px-3 py-2.5" /></label><label className="block text-sm"><span className="mb-1 block font-medium">End date</span><input disabled={!leaveTypes.length} required type="date" value={form.end_date} onChange={(event) => setForm({ ...form, end_date: event.target.value })} className="w-full rounded-lg border px-3 py-2.5" /></label>{form.duration === 'hours' && <label className="block text-sm"><span className="mb-1 block font-medium">Hours</span><input disabled={!leaveTypes.length} required type="number" min="0.5" max="24" step="0.5" value={form.hours} onChange={(event) => setForm({ ...form, hours: event.target.value })} className="w-full rounded-lg border px-3 py-2.5" /></label>}</div><textarea disabled={!leaveTypes.length} required minLength="3" placeholder="Reason" value={form.reason} onChange={(event) => setForm({ ...form, reason: event.target.value })} className="mt-4 w-full rounded-lg border px-3 py-2.5" rows="3" /><button disabled={!leaveTypes.length} type="submit" className="mt-4 rounded-lg bg-orange-500 px-4 py-2.5 font-semibold text-white disabled:opacity-50">Submit application</button></form>}
